...
export function validateId(id: string): string {
  if (!id.trim().length) {
    return error(400, `Group ID '${id}' is empty`);
  }
  if (!idValidatorRegex.test(id)) {
    return error(400, `Group ID '${id}' is contains illegal characters`);
  }
  if (id.startsWith('.')) {
    return error(400, `Group ID '${id}' has a leading dot`);
  }
  if (id.endsWith('.')) {
    return error(400, `Group ID '${id}' has a trailing dot`);
  }
  if (id.startsWith('-')) {
    return error(400, `Group ID '${id}' has a leading dash`);
  }
  if (id.endsWith('-')) {
    return error(400, `Group ID '${id}' has a trailing dash`);
  }
  return id;
}
